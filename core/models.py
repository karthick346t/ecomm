...
class Item(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:  # ✅ Generate slug only if it's empty
            ...
            while Item.objects.filter(slug=unique_slug).exists():  # ✅ Check if slug already exists
    ...
    def get_average_rating(self):
        reviews = self.reviews.all()  # Fetch all reviews related to this item
        if reviews.exists():
            avg_rating = round(sum(review.rating for review in reviews) / len(reviews), 1)
            print(f"DEBUG: Average rating for {self.title} = {avg_rating}")  # ✅ Debugging
            return avg_rating
        print(f"DEBUG: No reviews yet for {self.title}")  # ✅ Debugging
        return 0
...
# ✅ Move Review Model AFTER the Item Model
